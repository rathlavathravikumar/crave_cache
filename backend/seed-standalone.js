...
const seedData = async () => {
    let mongoServer;
    try {
        // Connect to the running server's database
        const envUri = process.env.MONGO_URI && process.env.MONGO_URI.trim();
        const uri = envUri || 'mongodb://127.0.0.1:27017/food-genie';
        ...
        if (mongoose.connection.readyState !== 1) {
            try {
                ...
                console.log('Primary connection failed, using in-memory MongoDB');
        ...
        for (const restaurant of createdRestaurants) {
            ...
            console.log(`Created menu and food items for ${restaurant.name}`);
        }
